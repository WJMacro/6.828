...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		...
		//call boot_alloc(0) to get the nextfree memory 
		//the memeories between nextfree and KERNBASE are already in use
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (pgdir[pdeID] == 0)
	{
		...
		if (page == NULL)
			return NULL;
		page->pp_ref += 1;
		pte_t pgEntry = (pte_t)page2pa(page)|PTE_P|PTE_U|PTE_W;
		pgdir[pdeID] = pgEntry;
	}
	pte_t pgEntry = pgdir[pdeID];
	pgEntry = pgEntry>>12<<12;
	int pteID =PTX(va);
	pte_t * pte =(pte_t*) pgEntry + pteID;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	{
		uintptr_t va_ = va + i*PGSIZE;
		pte_t* entry = pgdir_walk(pgdir, (const void*)va_, 1);
		if (entry == NULL)
			panic("pgdir_walk returns NULL!");
		physaddr_t pa_ = pa + i*PGSIZE;
		*entry = pa_|perm|PTE_P;
	}
}
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	if(entry == NULL)
		return -E_NO_MEM;
	if( (entry[0] &  ~0xfff) == page2pa(pp))
		pp->pp_ref--;
	else if(*entry != 0)
		page_remove(pgdir, va);
 
	*entry = (page2pa(pp) & ~0xfff) | perm | PTE_P;
	pp->pp_ref++;
	return 0;
}
